...
serve(async (req) => {
  ...
  try {
    ...
    const { analysis_id, analysis_result } = await req.json()

    console.log('Received report generation request:', { analysis_id })

    // Call external webhook
    const webhookUrl = Deno.env.get('GENERATE_REPORT_WEBHOOK_URL')
